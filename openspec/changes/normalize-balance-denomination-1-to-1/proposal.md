...
- 对唯一历史 `1:5` 充值账户 `s928215036@gmail.com`，仅充值资产与余额订单除以 `5`；其使用费率和使用记录仍按全站口径除以 `10`。
...
- Business baseline confirmed by operator: effective historical denomination is `1:10` for normal accounts; only `s928215036@gmail.com` uses the explicit `1:5` recharge-asset exception.
