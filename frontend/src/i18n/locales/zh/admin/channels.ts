export default {
    ...
    channels: {
      title: '售价策略',
      description: '按分组配置模型售价。选中左侧分组后，在本页直接编辑售价并保存。',
      workspaceHint: '选择左侧分组套餐 → 在右侧直接配置售价。无需再打开超大弹窗。',
      searchGroups: '搜索分组…',
      allPlatforms: '全部平台',
      noGroupsMatch: '没有匹配的分组',
      selectGroupPrompt: '请选择左侧分组',
      selectGroupHint: '进入后先选分组，再在本页配置售价策略与模型单价。',
      groupRateLine: '分组倍率 {rate}x',
      boundPolicy: '策略：{name}',
      followOfficial: '改为跟官方',
      savePricing: '保存售价',
      defaultPolicyName: '{group} 售价',
      officialOnlyWorkspace: '当前未绑定有效售价。添加模型售价并保存后，将自动为该分组创建/绑定策略。',
      sharedPolicyTitle: '⚠️ 共享策略（影响多个分组）',
      sharedPolicyDesc: '本策略同时绑定：{names}。在此保存会同步更新所有这些分组的售价。',
      sharedPolicyHint: '只想改当前分组？先「拆成本分组专属」再保存。',
      splitDedicated: '拆成本分组专属',
      splitConfirm: '将基于当前售价为本分组创建独立策略并绑定，不再影响其他分组。确定继续？',
      splitSuccess: '已拆分为本分组专属策略',
      splitError: '拆分失败',
      previewNote: '预览为已保存状态；改价后请点「保存售价」再刷新查看。',
      needPricingToCreate: '请至少添加一条模型售价后再保存',
      loadGroupsError: '加载分组失败',
      emptyModelsInPricing: '有定价条目未填写模型',
      noGroupsSelected: '平台 {platform} 未选择分组',
      searchChannels: '搜索售价策略...',
      createChannel: '创建售价策略',
      editChannel: '编辑售价策略',
      deleteChannel: '删除售价策略',
      statusActive: '启用',
      statusDisabled: '停用',
      allStatus: '全部状态',
      groupsUnit: '个分组',
      ...
      deleteConfirm: '确定要删除售价策略「{name}」吗？已绑定分组将改回跟官方基准。此操作不可撤销。',
      columns: {
        name: '名称',
        description: '描述',
        status: '状态',
      ...
      form: {
        name: '名称',
        namePlaceholder: '输入策略名称（如 luna×4）',
        description: '描述',
        ...
        scaleOfficialPrice: '按官方价',
        applyScale: '应用',
        customScalePlaceholder: 'N',
        clearPriceOverride: '清除覆盖（回退官方）',
        quickPricingLoading: '处理中…',
        tokenPriceHint: '单位 $ / 1M tokens。留空 = 使用官方基准价（不写死售价）。',
        imageOutputNullHint: '注意：命中本规则时图片输出留空会按 0 计，不会回退官方',
        fallbackOfficialShort: '回退官方',
        accountCostDefaultHint: '默认用官方基准算账号成本；打开开关会把「用户标价（倍率前）」当作账号成本。',
        addPlatform: '添加平台',
        ...
        applyPricingToAccountStatsDesc: '启用后，未被自定义规则匹配的请求将使用本次用户标价（分组倍率前）计算账号统计费用',
        accountStatsPricingRules: '自定义账号统计定价规则',
        addRule: '添加规则',
        noRulesConfigured: '未配置自定义规则：账号成本默认跟官方基准（除非打开上方开关）。',
        ruleName: '规则名称（可选）',
        ruleGroups: '分组',
        ruleAccounts: '账号',
        ...
        noGroupsInChannel: '请先在「基础设置」选择分组',
        unnamed: '未命名',
        syncLatestModels: '同步最新模型',
        syncingModels: '同步中...',
        syncModelsSuccess: '已同步 {count} 个新模型',
        syncModelsFilledSuccess: '已同步 {count} 个新模型（其中 {filled} 个已填官方价）',
    ...
    riskControl: {
      ...
      summary: {
        localPreBlock: '本地前置拦截',
        upstreamPolicy: '上游策略拦截',
        errors: '审核异常',
        total: '审核记录合计',
      },
      elevatedUserCol: '用户',
      elevatedUsers: '当前高危用户',
      elevatedUsersHint: '处于强制全量审核窗口的用户（违规/cyber 命中后自动加入）。',
      elevatedUsersEmpty: '当前没有高危用户',
      elevatedUsersCount: '{count} 人',
      elevatedTTL: '剩余 {time}',
      elevatedLastAction: '最近动作',
      elevatedForceSample: '强制全量审',
      elevatedUnknownUser: '未知用户',
      saveConfig: '保存内容审计配置',
      ...
      lightHitAction: '轻微触发：禁用当前 Key',
      lightHitActionHint: '开启后，单次有效命中会先禁用发起请求的 API Key，不影响同一用户的其他 Key。',
      repeatedHitAction: '多次触发：禁用用户',
      repeatedHitActionHint: '在累计窗口内达到阈值后，升级为禁用整个用户账号并失效认证缓存。',
      apiKeyDisabledEvent: 'Key 已禁用',
      enforcementFailed: '自动处置未完成',
