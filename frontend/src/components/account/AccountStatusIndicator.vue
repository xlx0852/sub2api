<template>
  <div class="flex items-center gap-2">
    ...
    <div v-if="hasError && errorTooltipText" class="group/error relative">
      <svg
        :class="[
          'h-4 w-4 cursor-help transition-colors',
          isGrokSpendingLimitBlocked
      ...
      <!-- Tooltip - 向下显示 -->
      <div
        class="invisible absolute left-0 top-full z-[100] mt-1.5 min-w-[200px] max-w-[300px] rounded-lg bg-gray-800 px-3 py-2 text-xs text-white opacity-0 shadow-lg transition-all duration-200 group-hover/error:visible group-hover/error:opacity-100 dark:bg-gray-900"
      >
...
const isGrokSpendingLimitBlocked = computed(() => {
  ...
  return (
    ...
    text.includes('personal-team-blocked:spending-limit') ||
...
const errorTooltipText = computed(() => {
  ...
  return props.account.error_message ?? ''
})
