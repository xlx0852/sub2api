<template>
  <div class="flex flex-col gap-0.5">
    <!-- 并发槽位 -->
    <CapacityBadge compact :color-class="concurrencyClass" :current="currentConcurrency" :max="account.concurrency">
    ...
    <!-- 5h窗口费用限制 -->
    <CapacityBadge v-if="showWindowCost" compact :color-class="windowCostClass" :tooltip="windowCostTooltip" :current="'$' + formatCost(currentWindowCost)" :max="'$' + formatCost(account.window_cost_limit)">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- 会话数量限制 -->
    <CapacityBadge v-if="showSessionLimit" compact :color-class="sessionLimitClass" :tooltip="sessionLimitTooltip" :current="activeSessions" :max="account.max_sessions!">
      <svg class="h-2.5 w-2.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <!-- RPM 限制 -->
    <CapacityBadge v-if="showRpmLimit" compact :color-class="rpmClass" :tooltip="rpmTooltip" :current="currentRPM" :max="account.base_rpm!" :suffix="rpmStrategyTag">
