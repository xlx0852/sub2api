<template v-if="active">
  <div class="space-y-4" data-testid="account-cost-config-panel">
    ...
    <div class="flex items-start justify-between gap-3 rounded-lg border border-gray-200 bg-gray-50 px-3 py-2 dark:border-dark-600 dark:bg-dark-700">
      <div class="min-w-0">
        ...
        <p class="mt-0.5 text-xs text-gray-500 dark:text-dark-400">
          {{ isSubscription ? t('admin.profit.costTypeAutoSubscription') : t('admin.profit.costTypeAutoMetered') }}
          <span v-if="isSubscription" class="block mt-0.5">{{ t('admin.profit.autoRenewHint') }}</span>
        </p>
      </div>
      <div v-if="isSubscription" class="flex shrink-0 flex-col items-end gap-1">
        <span class="text-[11px] font-medium text-gray-500 dark:text-dark-300">{{ t('admin.profit.autoRenew') }}</span>
        <button
          type="button"
