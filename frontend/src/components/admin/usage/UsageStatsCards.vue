<template>
  <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4 lg:grid-cols-4">
    <div class="card flex min-w-0 items-center gap-3 p-4">
      <div class="shrink-0 rounded-lg bg-blue-100 p-2 text-blue-600 dark:bg-blue-900/30">
    ...
    <div class="card flex min-w-0 items-center gap-3 p-4">
      ...
      <div class="min-w-0 flex-1">
        ...
        <p class="flex flex-wrap items-center gap-x-1 gap-y-0.5 text-xs leading-5 text-gray-500">
          ...
          <span class="group relative inline-flex cursor-help items-center gap-0.5" tabindex="0">
            ...
            <span
              class="pointer-events-none absolute right-0 top-full z-30 mt-2 w-56 max-w-[calc(100vw-2rem)] rounded-lg border border-gray-200 bg-white p-3 text-left text-xs text-gray-700 opacity-0 shadow-lg transition-opacity duration-150 group-hover:opacity-100 group-focus:opacity-100 sm:left-1/2 sm:right-auto sm:-translate-x-1/2 dark:border-dark-600 dark:bg-dark-800 dark:text-dark-200"
            >
    ...
    <div class="card flex min-w-0 items-center gap-3 p-4">
      <div class="shrink-0 rounded-lg bg-green-100 p-2 text-green-600 dark:bg-green-900/30">
    ...
    <div class="card flex min-w-0 items-center gap-3 p-4">
      <div class="shrink-0 rounded-lg bg-purple-100 p-2 text-purple-600 dark:bg-purple-900/30">
        <Icon name="clock" size="md" />
      </div>
      <div class="min-w-0 flex-1"><p class="text-xs font-medium text-gray-500">{{ t('usage.avgDuration') }}</p><p class="text-xl font-bold">{{ formatDuration(stats?.average_duration_ms || 0) }}</p></div>
    </div>
  </div>
</template>
