<template>
  ...
  <Teleport to="body">
    ...
    >
      <div class="whitespace-nowrap rounded-lg border border-gray-700 bg-gray-900 px-3 py-2.5 text-xs text-white shadow-lg dark:border-gray-600 dark:bg-gray-800">
...
import { isUsageWSHTTPBridgeReplay, resolveUsageRequestType } from '@/utils/usageRequestType'
import {
  BILLING_MODE_TOKEN,
  getBillingModeLabel,
...
const getRequestTypeLabel = (row: AdminUsageLog): string => {
  if (isUsageWSHTTPBridgeReplay(row)) return t('usage.wsBridgeReplay')
...
const getRequestTypeBadgeClass = (row: AdminUsageLog): string => {
  if (isUsageWSHTTPBridgeReplay(row)) return 'bg-fuchsia-100 text-fuchsia-800 dark:bg-fuchsia-900 dark:text-fuchsia-200'
  const requestType = resolveUsageRequestType(row)
  if (requestType === 'cyber') return 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200'
  if (requestType === 'compact') return 'bg-emerald-100 text-emerald-800 dark:bg-emerald-900 dark:text-emerald-200'
