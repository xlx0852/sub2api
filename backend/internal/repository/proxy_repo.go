...
func (r *proxyRepository) SweepExpiredProxies(ctx context.Context, now time.Time) (int64, error) {
	...
	if len(changedAccountIDs) > 0 {
		...
		// 改投已落库，outbox 失败会让调度缓存继续用旧 proxy，加重试降低窗口。
		payload := map[string]any{"account_ids": changedAccountIDs}
		if err := enqueueSchedulerOutboxWithRetry(ctx, r.sql, service.SchedulerOutboxEventAccountBulkChanged, nil, nil, payload, 3); err != nil {
			logger.LegacyPrintf("repository.proxy", "[SchedulerOutbox] enqueue proxy expiry account changes failed after retries: err=%v account_ids=%v", err, changedAccountIDs)
		}
	}
