...
// TLS 指纹 Transport 默认不挂 h2 keepalive（Claude 等 ALPN=http/1.1）。
// OpenAI 指纹路径由调用方在 protocolMode=openai_h2 时再 enableOpenAIHTTP2KeepAlive。
func TestBuildUpstreamTransportWithTLSFingerprint_DefaultNoEagerH2KeepAlive(t *testing.T) {
	tr, err := buildUpstreamTransportWithTLSFingerprint(http2KeepAliveTestPoolSettings(), nil, nil)
	require.NoError(t, err)
	require.False(t, tr.ForceAttemptHTTP2)
	require.Nil(t, tr.TLSNextProto["h2"], "指纹 Transport 构建期不应主动配置 http2 keepalive")

	// 模拟 OpenAI 指纹路径：在 DialTLS 之外再挂 keepalive。
