...
func (m *openAIWSPassthroughUsageMeta) initFromFirstFrame(policyOutput []byte, mappedModel string) {
	...
	m.reasoningEffort.Store(extractOpenAIReasoningEffortFromBody(policyOutput, firstNonEmpty(mappedModel, m.sessionRequestModel)))
}
...
func (m *openAIWSPassthroughUsageMeta) updateFromResponseCreate(policyOutput []byte, mappedModel string, requestModelForFrame string) {
	...
	m.serviceTier.Store(extractOpenAIServiceTierFromBody(policyOutput))
	m.reasoningEffort.Store(extractOpenAIReasoningEffortFromBody(policyOutput, firstNonEmpty(mappedModel, requestModelForFrame)))
}
		...
		Options: openaiwsv2.RelayOptions{
			...
			OnTurnComplete: func(turn openaiwsv2.RelayTurnResult) {
				...
				turnResult := &OpenAIForwardResult{
					...
					BillingRequestID: "ws-turn:" + generateRequestID(),
					Usage: OpenAIUsage{
