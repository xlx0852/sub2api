...
func TestBillingService_GPT56CacheWritePricingUsesOfficialMultiplier(t *testing.T) {
	tests := []struct {
		...
		{model: "gpt-5.6-luna", input: 0.4e-6, inputPriority: 0.8e-6, output: 2.4e-6, outputPriority: 4.8e-6, cacheRead: 0.04e-6, cacheReadPriority: 0.08e-6},
	}
...
func TestBillingService_GPT56UsesLongContextPricingAcrossModelsAndTiers(t *testing.T) {
	models := []struct {
		...
		{name: "gpt-5.6-luna", input: 0.4e-6, cached: 0.04e-6, cacheWrite: 0.5e-6, output: 2.4e-6},
	}
	tiers := []struct {
		name       string
		priceScale float64
...
func TestPricingService_BareGPT56AliasDeterministicallyUsesSol(t *testing.T) {
	pricingSvc := &PricingService{pricingData: map[string]*LiteLLMModelPricing{
		...
		"gpt-5.6-luna":  {InputCostPerToken: 0.4e-6},
...
func TestDefaultPricingIncludesOfficialGPT56Rates(t *testing.T) {
	...
	tests := []struct {
		...
		{model: "gpt-5.6-luna", input: 0.4e-6, cached: 0.04e-6, cacheWrite: 0.5e-6, output: 2.4e-6, inputPriority: 0.8e-6, cachedPriority: 0.08e-6, cacheWritePriority: 1e-6, outputPriority: 4.8e-6},
	}
	for _, tt := range tests {
		t.Run(tt.model, func(t *testing.T) {
...
func TestGPT56DedicatedFallbacksUseOfficialRates(t *testing.T) {
	tests := []struct {
		...
		{model: "gpt-5.6-luna", input: 0.4e-6, cached: 0.04e-6, cacheWrite: 0.5e-6, output: 2.4e-6},
	}
...
func TestBundledPricingContainsGrokFallbackOnlyModels(t *testing.T) {
	data, err := os.ReadFile(filepath.Join("..", "..", "resources", "model-pricing", "model_prices_and_context_window.json"))
	require.NoError(t, err)

	svc := &PricingService{}
	pricing, err := svc.parsePricingData(data)
	require.NoError(t, err)
	...
	for _, model := range []string{
		...
		entry := pricing[model]
		require.NotNil(t, entry, "missing bundled pricing for %s", model)
		require.Positive(t, entry.OutputCostPerImage, "missing output unit price for %s", model)
	}

	composer := pricing["grok-composer-2.5-fast"]
	require.NotNil(t, composer)
	require.Positive(t, composer.InputCostPerToken)
	require.Positive(t, composer.OutputCostPerToken)
}
