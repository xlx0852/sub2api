...
func TestParseUsageRequestType(t *testing.T) {
	...
	cases := []testCase{
		...
		{name: "compact", input: "compact", want: RequestTypeCompact},
...
func TestRequestTypeNormalizeAndString(t *testing.T) {
	...
	require.Equal(t, "compact", RequestTypeCompact.String())
}
...
func TestApplyLegacyRequestFields(t *testing.T) {
	...
	stream, ws = ApplyLegacyRequestFields(RequestTypeCompact, true, true)
	require.False(t, stream)
	require.False(t, ws)
