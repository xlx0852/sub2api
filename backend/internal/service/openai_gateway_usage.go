...
func (s *OpenAIGatewayService) RecordUsage(ctx context.Context, input *OpenAIRecordUsageInput) error {
	...
	cost, err = s.calculateOpenAIRecordUsageCost(ctx, result, apiKey, billingModels, multiplier, imageMultiplier, videoMultiplier, tokens, serviceTier, longContextBillingEnabled)
	if err != nil {
...
func (s *OpenAIGatewayService) calculateOpenAIRecordUsageCost(
	...
	if result != nil && isGrokVideoUsageResult(result, billingModels) {
...
func apiKeyHasConfiguredImagePrice(apiKey *APIKey, sizeTier string) bool {
	cfg := imagePriceConfigFromAPIKey(apiKey)
	if cfg == nil {
		return false
	}
...
func apiKeyHasConfiguredVideoPrice(apiKey *APIKey, resolution string) bool {
	cfg := videoPriceConfigFromAPIKey(apiKey)
	if cfg == nil {
		return false
	}
