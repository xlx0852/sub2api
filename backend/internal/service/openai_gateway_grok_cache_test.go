...
func TestGrokCacheIdentityStableAcrossAccountFailover(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c := newGrokCacheTestContext(302)
	c.Request.Header.Set("session_id", "stable-failover-session")
	body := []byte(`{"model":"grok","input":"same request"}`)
	identity := resolveGrokCacheIdentity(c, body, "", "grok-4.5")
	require.NotEmpty(t, identity)

	firstAccount := &Account{Platform: PlatformGrok, Type: AccountTypeAPIKey, Credentials: map[string]any{"base_url": "https://first.example.test", "using_api": true}}
	secondAccount := &Account{Platform: PlatformGrok, Type: AccountTypeAPIKey, Credentials: map[string]any{"base_url": "https://second.example.test", "using_api": true}}
	firstRequest, err := buildGrokResponsesRequest(c.Request.Context(), c, firstAccount, body, "token-1", identity)
	require.NoError(t, err)
	secondRequest, err := buildGrokResponsesRequest(c.Request.Context(), c, secondAccount, body, "token-2", identity)
	require.NoError(t, err)

	require.NotEqual(t, firstRequest.URL.Host, secondRequest.URL.Host)
	require.Equal(t, identity, firstRequest.Header.Get(grokConversationIDHeader))
	require.Equal(t, firstRequest.Header.Get(grokConversationIDHeader), secondRequest.Header.Get(grokConversationIDHeader))
}
...
func TestGrokConversationHeaderDoesNotBecomeStickySession(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Empty(t, (&OpenAIGatewayService{}).ExtractSessionID(grokContext, body))

	withoutGrokHeader := newGrokCacheTestContext(601)
	require.Equal(t,
		(&OpenAIGatewayService{}).GenerateSessionHash(withoutGrokHeader, body),
		(&OpenAIGatewayService{}).GenerateSessionHash(grokContext, body),
	)
}
