...
func resolveOpenAIForwardModel(account *Account, requestedModel, messagesDispatchMappedModel string) string {
	...
	isMessagesModel := strings.HasPrefix(strings.ToLower(strings.TrimSpace(requestedModel)), "claude")
	if account == nil {
		if isMessagesModel && messagesDispatchMappedModel != "" {
	...
	if !matched && isMessagesModel && messagesDispatchMappedModel != "" {
		return messagesDispatchMappedModel
	}
	return mappedModel
}
