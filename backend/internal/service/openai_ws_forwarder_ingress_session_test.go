...
func TestOpenAIGatewayService_ProxyResponsesWebSocketFromClient_RetriesIndexedCompatibilityOnce(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	firstConn := &openAIWSCaptureConn{events: [][]byte{
		[]byte(`{"type":"error","error":{"type":"invalid_request_error","code":"unknown_parameter","message":"Unknown parameter: 'input[0].arguments'.","param":"input[0].arguments"}}`),
	}}
	secondConn := &openAIWSCaptureConn{events: [][]byte{
		[]byte(`{"type":"response.completed","response":{"id":"resp_indexed_retry","model":"gpt-5.5","output":[],"usage":{"input_tokens":1,"output_tokens":1}}}`),
	}}
	...
	account := &Account{
		ID:          11801,
		Name:        "openai-ingress-indexed-retry",
		Platform:    PlatformOpenAI,
		Type:        AccountTypeAPIKey,
		Status:      StatusActive,
		Schedulable: true,
	...
	payload := `{"type":"response.create","model":"gpt-5.5","stream":false,"input":[
	...
	require.Equal(t, "resp_indexed_retry", gjson.GetBytes(message, "response.id").String())
	...
	case serverErr := <-serverErrCh:
		require.NoError(t, serverErr)
	case <-time.After(5 * time.Second):
		t.Fatal("等待 indexed compatibility retry ingress websocket 结束超时")
	}
	...
	require.Equal(t, "custom_tool_call", gjson.Get(requestToJSONString(firstWrites[0]), "input.0.type").String())

	secondConn.mu.Lock()
	secondWrites := append([]map[string]any(nil), secondConn.writes...)
	secondConn.mu.Unlock()
	require.Len(t, secondWrites, 1)
	secondPayload := requestToJSONString(secondWrites[0])
	require.Equal(t, "function_call", gjson.Get(secondPayload, "input.0.type").String())
	require.JSONEq(t, `{"input":"x"}`, gjson.Get(secondPayload, "input.0.arguments").String())
	require.Equal(t, "function_call_output", gjson.Get(secondPayload, "input.1.type").String())
	require.Equal(t, "tool_search_call", gjson.Get(secondPayload, "input.2.type").String())
	require.False(t, gjson.Get(secondPayload, "input.0.id").Exists())
	require.False(t, gjson.Get(secondPayload, "input.1.id").Exists())
}
