...
func TestOpenAIResponses_AcceptsHTTPPreviousResponseIDForAffinity(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	// previous_response_id is no longer rejected at validation for HTTP affinity.
	// Without available accounts the request fails later (not a 400 protocol rejection).
...
func TestOpenAIResponsesWebSocket_SetsClientTransportWSWhenUpgradeValid(t *testing.T) {
	...
	require.NotContains(t, w.Body.String(), "websocket_not_supported")
}

func TestOpenAIResponsesWebSocket_GrokReturns426BeforeHandshake(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	require.Contains(t, w.Body.String(), "websocket_not_supported")
	require.Contains(t, w.Body.String(), "HTTP Responses/SSE")
	require.Equal(t, service.OpenAIClientTransportWS, service.GetOpenAIClientTransport(c))
}
