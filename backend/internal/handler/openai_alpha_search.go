...
func (h *OpenAIGatewayHandler) AlphaSearch(c *gin.Context) {
	...
	for {
		...
		accountRelease, acquired := h.acquireResponsesAccountSlot(c, apiKey.GroupID, sessionHash, selection, requestedModel, false, &streamStarted, reqLog)
		if !acquired {
			return
		}
		...
		if errors.Is(err, context.Canceled) {
			if c.Writer.Size() == writerSizeBeforeForward {
				status, errType, message := concurrencyErrorResponse(err, "request")
				h.errorResponse(c, status, errType, message)
			}
			reqLog.Info("openai_alpha_search.client_canceled", zap.Int64("account_id", account.ID))
			return
		}
