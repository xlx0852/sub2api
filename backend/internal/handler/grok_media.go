...
// GrokVideoEdit handles xAI video editing through Grok groups.
...
func (h *OpenAIGatewayHandler) handleGrokMedia(c *gin.Context, endpoint service.GrokMediaEndpoint, requestID string) {
	...
	for {
		...
		accountReleaseFunc, accountAcquired := h.acquireResponsesAccountSlot(c, apiKey.GroupID, sessionHash, selection, requestModel, false, &streamStarted, reqLog)
		if !accountAcquired {
			return
		}
		...
		if shouldBindGrokMediaVideoRequest(endpoint, result.ResponseID) {
...
func shouldBindGrokMediaVideoRequest(endpoint service.GrokMediaEndpoint, responseID string) bool {
	return (endpoint == service.GrokMediaEndpointVideosGenerations || endpoint == service.GrokMediaEndpointVideosEdits) &&
