...
func RegisterAdminRoutes(
	...
	admin.Use(middleware.AdminComplianceGuard(settingService))
	{
		...
		admin.GET("/dashboard/availability", h.Admin.Dashboard.GetTrafficAvailability)
...
func registerProfitRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	profit.POST("/cycles/:id/termination", h.Admin.Profit.CreateSubscriptionTermination)
	profit.POST("/cycles/:id/termination-preview", h.Admin.Profit.PreviewSubscriptionTermination)
	profit.POST("/terminations/:id/refunds", h.Admin.Profit.CreateSubscriptionRefund)
	profit.POST("/terminations/:id/reverse", h.Admin.Profit.ReverseSubscriptionTermination)
...
func registerAccountRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	accounts := admin.Group("/accounts")
	{
		...
		accounts.POST("/performance-stats/batch", h.Admin.Account.GetBatchPerformanceStats)
		...
		accounts.POST("/cleanup-oauth-email-duplicates", h.Admin.Account.CleanupOAuthEmailDuplicates)
...
func registerScheduledTestRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	...
	admin.POST("/accounts/:id/scheduled-test-plans/ensure-default", h.Admin.ScheduledTest.EnsureDefault)
}
