...
func TestGatewayRoutesNonGrokVideosAreRejectedAtPlatformGate(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{http.MethodPost, "/v1/videos/edits", `{"model":"grok-imagine-video-1.5","prompt":"add waves"}`},
		{http.MethodPost, "/videos/edits", `{"model":"grok-imagine-video-1.5","prompt":"add waves"}`},
...
func TestGatewayRoutesGrokResponsesWebSocketFallsBackBeforeHandshake(t *testing.T) {
	router := newGatewayRoutesTestRouter(service.PlatformGrok)
	...
	for _, path := range []string{
		...
		require.Equal(t, http.StatusUpgradeRequired, w.Code, "path=%s", path)
		require.NotEqual(t, http.StatusSwitchingProtocols, w.Code, "path=%s", path)
		require.Contains(t, w.Body.String(), "websocket_not_supported", "path=%s", path)
		require.Contains(t, w.Body.String(), "HTTP Responses/SSE", "path=%s", path)
	}
}
